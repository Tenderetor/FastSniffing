...
void Send_To_The_Internet()
{
    ...
    if (doc_how_many_times == 0)
    {
        Serial.println();

        Serial.print("SD Save... ");
        SaveToSDCard(Json_output_string[0]);
        Serial.println();

        Serial.println("doc_how_many_times == 0");
        Serial.print("Post... ");
        Serial.println(Json_output_string[0]);

        //Serial.println(http.post("gjm.yjg.mybluehost.me/kptbot.php", output, "application/json"));
        PostResult = http.post("gjm.yjg.mybluehost.me/kptbot.php", Json_output_string[0], "application/json");
        Serial.println(PostResult);
        post_status(PostResult);
    ...
    {
        //for (counter = 0; counter < (doc_how_many_times); counter++)
        for(int u=0; u<times_we_loaded_output-1; u++)
        {
            Serial.println("The counter to loop the output array is at");
            //Serial.println(counter);
            Serial.println(u);
            Serial.println();

            Serial.println("SD Save... ");
            //SaveToSDCard(Json_output_string[counter]);
            //Serial.println(Json_output_string[counter]);
            SaveToSDCard(Json_output_string[u]);
            Serial.println(Json_output_string[u]);
            ...
            Serial.println(Json_output_string[u]);
            //Serial.println(http.post("gjm.yjg.mybluehost.me/kptbot.php", output, "application/json"));
            //PostResult = http.post("gjm.yjg.mybluehost.me/kptbot.php", Json_output_string[counter], "application/json");
            PostResult = http.post("gjm.yjg.mybluehost.me/kptbot.php", Json_output_string[u], "application/json");
            Serial.println(PostResult);
            post_status(PostResult);
            ...
            if (!SuccessfulPostFlagg)
            {
                Serial.println("POST FAILED. now saving data for later POST trying...");
                atleast_one_post_failed = true; //maybe test after a time (maybe end of day)if this flag is true and then act
                //PostOutputLater[later_counter]=Json_output_string[counter]; //output is a long string with many data
                Serial.println("The following failed while and has been saved for later save/post");
                //Serial.println(PostOutputLater[later_counter]);
                //later_counter++;
                Serial.println("======================================================================================");
        ...
        {
            Serial.println("We have a remainder to post");
            Serial.println("The counter to loop the output array is now at");
            //Serial.println(counter);
            Serial.println(mini_global_u+1);
            Serial.println();

            Serial.println("Remainder SD Save... ");
            //SaveToSDCard(Json_output_string[counter]);
            Serial.println(Json_output_string[mini_global_u+1]);
            SaveToSDCard(Json_output_string[mini_global_u+1]);
            ...
            //Serial.println(http.post("gjm.yjg.mybluehost.me/kptbot.php", output, "application/json"));
            //PostResult = http.post("gjm.yjg.mybluehost.me/kptbot.php", Json_output_string[counter], "application/json");
            PostResult = http.post("gjm.yjg.mybluehost.me/kptbot.php", Json_output_string[mini_global_u+1], "application/json");
            Serial.println(PostResult);
            post_status(PostResult);
            ...
            if (!SuccessfulPostFlagg)
            {
                Serial.println("POST FAILED. now saving data for later POST trying...");
                atleast_one_post_failed = true; //maybe test after a time (maybe end of day)if this flag is true and then act
                //PostOutputLater[later_counter]=output; //output is a long string with many data
                Serial.println("The following failed while and has been saved for later save/post");
                //Serial.println(PostOutputLater[later_counter]);
                //later_counter++;
                Serial.println("======================================================================================");
