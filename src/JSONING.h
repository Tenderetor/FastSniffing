...
extern bool finished_recursion_flagg;
...
void AlldomumentingJson()
{
  ...
  Serial.println("The documentingJson function");
    ...
    //for (int k = loop_start; k < (loop_end / 2); k++)
    /*
    for(int k = 0; k < sniffed_macs_global_counter; k++)
    {
      JsonObject root = doc.createNestedObject();
      Serial.print("The current address: ");
      Serial.println(Sniffed_Mac_Addresses[k]);

      root["addr"].set(Sniffed_Mac_Addresses[k]); //(periphery_address[k]);
      root["lat"].set(latitude);
      root["lon"].set(longitude);
      root["time"].set(Tym);
